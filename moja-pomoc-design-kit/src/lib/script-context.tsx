...
type Ctx = { script: Script; setScript: (s: Script) => void; t: (v: { sr_lat: string; sr_cyr: string }) => string };
...
export function latToCyr(s: string): string {
  ...
  while (i < lower.length) {
    ...
    if (pairs[two]) { out += pairs[two]; i += 2; continue; }
    const ch = lower[i];
...
export function ScriptProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const v = (typeof window !== "undefined" && localStorage.getItem("mp_script")) as Script | null;
  ...
  return <ScriptContext.Provider value={{ script, setScript, t }}>{children}</ScriptContext.Provider>;
}

export function useScript() {
  const v = useContext(ScriptContext);
  if (!v) {
    // Safe fallback (used during SSR splitting / HMR transitions)
    return {
